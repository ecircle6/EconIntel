...
8 源采集（RSS/API/HTML，全部免维护）
  ...
  → 0-100 重要性评分（机构权威 32 + 引用 30 + 时效 26 + 论文类型 12，权重可配，1 位小数）
  → AI 精简（LLM 优先，规则降级）
  → 导出静态站点（滚动分片）
```
...
- 公式：`机构权威(A=32/B=20/C=8) + 引用数(log1p 映射，100 引用打满) + 时效(连续指数衰减) + 论文类型(期刊 12/工作论文 4)`，总分 1 位小数
- 论文详情页展示**评分构成**（逐项分数 + 引用数来源与数值），状态页展示 🔥/⭐/📄 分布占比
- 权重全部可通过环境变量调整（`EI_W_*`）；标签阈值 80/60 固定（对应 🔥/⭐）
