...
一个**网址**，每 6 小时云端自动更新。任何人打开链接即可浏览最新经济学论文——**零安装、零环境依赖、手机电脑都能用**。
...
之后：**每 6 小时自动更新**（北京时间 2/8/14/20 点；GitHub cron 实际可能有 5-30 分钟延迟，页面显示真实更新时间）；`--if-stale`（最近 `EI_STALE_HOURS` 小时，默认 5 小时）机制防止重复运行；数据库随每次更新提交回仓库，兼作**异地备份**。前端打开期间每 60 秒检测一次数据版本，发现新数据自动提示刷新。
...
8 源采集（RSS/API/HTML，全部免维护）
  ...
  → 引用数（CrossRef / OpenAlex，带标题相似度+年份校验防同名误配）→ 领域分类（规则词典，标题加权）
  → 0-100 重要性评分（机构权威 40 + 引用 23 + 时效 25 + 论文类型 12，权重可配，1 位小数）
  → AI 精简（LLM 优先，规则降级）
  → 导出静态站点（滚动分片）
```
...
- **引用数真实性校验**：DOI 精确匹配直接采纳；标题检索必须通过 token 重叠率 ≥0.5 且年份差 ≤2 的校验，防同名旧论文误配；无来源引用一律视为「未知」（按 0 计并在详情页标注），绝不虚构
- **OpenAlex 已商业化（2026-08 实测）**：每日免费额度极少，429 响应含 `Retry-After`，用尽后自动熔断本轮、次日额度重置自动续跑；有 DOI 的论文（NBER/期刊等）走 CrossRef 免费引用计数，不受影响
- **GitHub cron 是 best-effort**：每 6 小时触发（北京 2/8/14/20 点），实际可能有 5-30 分钟延迟——页面显示真实生成时间
- **摘要缺失不编造**：展示「该来源未提供摘要」降级样式；状态页按源展示摘要覆盖率指标（实测 8 源总体 ≥97%）

### 重要性评分透明性

- 公式：`机构权威(A=40/B=24.8/C=10) + 引用数(log1p 映射，100 引用打满) + 时效(连续指数衰减) + 论文类型(期刊 12/工作论文 4)`，总分 1 位小数
- 论文详情页展示**评分构成**（逐项分数 + 引用数来源与数值），状态页展示 🔥/⭐/📄 分布占比
- 权重全部可通过环境变量调整（`EI_W_*`）；标签阈值 68/50（对应 🔥/⭐）：A 档官方机构（NBER/CEPR/美联储等）当天发布的论文 ≈69 分自然进入热点，老论文可靠引用翻盘，B 档当天期刊 ≈62 分进入重要
